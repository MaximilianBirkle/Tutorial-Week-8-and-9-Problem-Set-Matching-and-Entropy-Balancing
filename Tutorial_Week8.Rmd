...
# Compute RMSPE
rmspe <- sqrt(mean((dataprep_out$Y1plot - dataprep_out$Y0plot %*% synth_out$solution.w)^2))
cat("Pre-treatment RMSPE:", round(rmspe, 2), "\n")
```
...
#Report post-treatment average effect (1990–2003)
post_years <- 1990:2003
avg_gap_post <- mean(gaps[rownames(dataprep_out$Y1plot) %in% post_years])
cat("Average post-1990 GDP per capita gap (West Germany - Synthetic):",
round(avg_gap_post, 2), "\n")
