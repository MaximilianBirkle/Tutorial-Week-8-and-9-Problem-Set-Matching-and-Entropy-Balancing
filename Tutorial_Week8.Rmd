...
m1 <- matchit(treat ~ 1,
              data        = dta_complete,
              distance    = dta_complete$logit_ps_logit,
              method      = "nearest",
              replace     = TRUE,
              ratio       = 1,
              caliper     = 0.2,
...
```{r q5-5-stats}
get_match_stats <- function(m_obj, ratio_m) {
  # Extract match matrix (rows = treated, cols = matched controls)
  mm <- m_obj$match.matrix
  
  # Number of treated units successfully matched = number of rows
  n_pairs <- nrow(mm)
  
  # Compute mean absolute distance in logit(PS) between each treated
  # and its matched controls
  dist_vec <- c()
  for (i in seq_len(nrow(mm))) {
    treated_id  <- rownames(mm)[i]
    control_ids <- mm[i, ]
    control_ids <- control_ids[!is.na(control_ids)]
    
    treat_logit   <- dta_complete[treated_id, "logit_ps_logit"]
    control_logit <- dta_complete[control_ids, "logit_ps_logit"]
    
    dist_vec <- c(dist_vec, abs(control_logit - treat_logit))
  }
  mean_dist <- mean(dist_vec, na.rm = TRUE)
  
  # Build matched dataset with weights + subclass IDs
  md <- match.data(m_obj, data = dta_complete)
  
  # Weighted regression for ATT
  fit <- lm(outcome ~ treat, data = md, weights = weights)
  
  # Cluster-robust VCOV clustered on subclass (matched set)
  vc  <- sandwich::vcovCL(fit, cluster = md$subclass)
  att <- coef(fit)["treat"]
  se  <- sqrt(diag(vc))["treat"]
  
  data.frame(m = ratio_m,
             N_matched_pairs  = n_pairs,
             mean_match_dist  = mean_dist,
             ATT  = att,
             Std_Error = se)}

# Apply helper to the three designs
results_ratios <- bind_rows(get_match_stats(m1, 1),
                            get_match_stats(m2, 2),
                            get_match_stats(m3, 3))

results_ratios
```

```{r q5-5-results-table}
results_ratios_table <- results_ratios %>%
  dplyr::mutate(
    `Matching Ratio (1:m)` = paste0("1:", m),
    `Mean Match Distance`  = round(mean_match_dist, 3),
    `ATT Estimate`         = round(ATT, 3),
...
# Let kable pick the right format for the current output (latex vs markdown)
tbl <- knitr::kable(results_ratios_table,
                    caption = "ATT Estimates by Matching Ratio (Logit PS, Caliper 0.2, With Replacement)",
                    booktabs = knitr::is_latex_output())  # only relevant for LaTeX/PDF


# Only apply kableExtra styling when knitting to LaTeX or HTML (not github_document)
if (knitr::is_latex_output() || knitr::is_html_output()) {
  tbl <- tbl %>%
...
ggplot(results_ratios, aes(x = m, y = ATT)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = ATT - 1.96 * Std_Error,
                    ymax = ATT + 1.96 * Std_Error),
                    width = 0.1) +
  scale_x_continuous(breaks = c(1, 2, 3),
                     labels = c("1:1", "1:2", "1:3")) +
  labs(title = "ATT by Matching Ratio (1:m)",
       x     = "Matching Ratio m (1:m)",
       y     = "ATT (UN Intervention Effect on Conflict Duration)") +
  theme_bw()
...
The pattern shows up in the Plot here. The standard errors get smaller from 1:1 over 2:1 to 3:1, meaning the estimates are theoretically getting better (variance decreases). However bias must be also increasing, because our ATT shifts from negative (1:1) over "slightly less negative" (2:1) to becoming positive (3:1). Our matching in this Data Set is not really powerful, as we established, meaning the second or third nearest member might be "very far away", increasing bias heavily.
...
Honestly, i do not think any of these designs produce useful causal estimates, but if one had to choose its probably the 1:1 option. It does have the most variance but the fact that we have very far distances and much bias when adding extra control, so were pulling in worse matches with 2:1 and 3:1. The results relate to Abadie & Imbens (2006) in the sense that we definitely see the variance-bias trade off pattern. Also, we only have a N of 154 observations and they state that bias gets smaller with larger numbers in N, so the low number of 154 could explain the terrible results. Therefore, with infinite data and perfect overlap, Bias would go to zero for all matching rations (X:1) and also the variance would continue to get smaller with rising ratios.
---
...
md_logit <- match.data(m_logit, data = dta_complete)

# Fit ATT regression using matching weights
fit_logit <- lm(outcome ~ treat, data = md_logit, weights = weights)

# Compute cluster-robust VCOV (clustered by matched-set "subclass")
vcov_logit <- vcovCL(fit_logit,
              cluster = md_logit$subclass,
              type = "HC3")     # recommended in the question
  

# Extract ATT and its SE
tau_logit <- coef(fit_logit)["treat"]
se_logit  <- sqrt(diag(vcov_logit))["treat"]
...
md_rf <- match.data(m_rf, data = dta_complete)

# Fit ATT regression
fit_rf <- lm(outcome ~ treat, data = md_rf, weights = weights)

# Cluster-robust SEs (cluster by matched sets)
vcov_rf <- vcovCL(fit_rf,
           cluster = md_rf$subclass,
           type = "HC3")

# Extract ATT and SE
tau_rf <- coef(fit_rf)["treat"]
se_rf  <- sqrt(diag(vcov_rf))["treat"]
...
# Compare results
q6_results <- data.frame(Method = c("Logit-matched", "RF-matched"),
                         ATT_Estimate = c(tau_logit, tau_rf),
                         Std_Error = c(se_logit, se_rf))

knitr::kable(q6_results,
             digits = 3,
             caption = "Q6: Cluster-Robust Inference for Logit and RF Matching (HC3)")

```

**Interpretation:**

The the two methods still produce the same very different point estimates, which comes down to poor overlap, as explained in Q5. The standard errors are both very large, RF = 21.97 and Logit = 29.45. The value is a little lower for RF, probably because they have  1 more matched control per treated unit. Overall, these huge standard errors reflect the earlier results that neither matched sample provides any reliable estimates of a causal effect. 
...
According to Rosenbaum & Rubin (1983) the propensity score is a balancing score because: $T_i \perp X_i \mid e(X_i)$. So within the "respective propensity score" the distribution of covariates is the same for treated and control units. So we "collapse" all relevant covariate information into one probability (scalar). So, like stated earlier, if treatment is independent of potential outcomes given covariates, then propensity score is enough to balance on. Once we match the units on propensity scores, they should look similar on all covariates. 

**2:**

The logistic regression is "smoother", interactions or nonlinarities have to be explicitly added. This theoretically gives less extreme predicted probabilities (which makes matching easier) but also tends to underfit relationships when they are somewhat complicated. A Random Forest estimation is more "flexible", meaning it can approximate complex nonlinearities automatically. However this might lead to overfitting simple relationships. 

**3:**

For the ATT we estimate: $\mathbb{E}[Y(0) \mid T = 1]$. It is required that control units look like treated units in terms of X, all others fall under overlap violations and will not be matched. So all treated units that fall outside the support of the controls have no comparable control units and the ATT becomes unidentifiable for that unit. When overlap is weak, like here, we have an unstable ATT, high bias, large standard errors and high sensitivity to matching design. Thus overlap is the basic requirement that ensures we construct a meaningful comparison group. 
...
**2. Suitability for West Germany**
...
**3. Key Identification Assumption**
...
The optimization problem in synthetic control is the problem of finding the ideal set of weights $w$ to build the most similar-looking control group to the treatment group pre-treatment. To answer the above questions in more detail, we first have to define the meaning of the underlying terminology. The core function is a minimization of a sum of squared differences (a form of Mean Squared Error) over the pre-intervention period ($t≤T_0$) between the treated unit's actual outcome ($Y_{1t}$) and the synthetic control's outcome ($∑w_j Y_{jt}$).

**The Synthetic Control**

The synthetic control term is what makes the whole summation look more complicated, although it is relatively straightforward. First of all, there is a sample of $J+1$ units of analysis (e.g., countries), where $j=1$ is the case of interest ("treated unit"), and the units ($j=2$ to $j=J+1$) define the "donor pool" of potential comparison units. The goal of the synthetic control is to find a weighted average of the donor pool, that is, a $(J \times 1)$ vector of weights $W= (w_2, ..., w_{J+1})$, with $0 \leq w_j \leq 1$ for $(j=2, ..., J+1)$ and $(w_2+...+w_{J+1}=1)$, such that the characteristics of the treated unit are best resembled by the synthetic control. 

Let $X_1$ be a $(k\times 1)$ vector of pre-intervention characteristics for the treated unit and $X_0$ be a $(k\times J)$ matrix of the same variables for the donor pool ($j=2$ to $j=J+1$). The optimization problem is defined as finding a $W^*$ that minimizes $\|X_1-X_0W\|$, the difference between the treated unit's characteristics and the synthetic control.
...
This expression calculates the weighted sum of squared differences between the treated unit's pre-intervention characteristics $(X_{1m})$ and the synthetic control's characteristics $(∑w_j X_{jm})$, where $W$ contains the weights $w_j$. The goal is to find the set of weights $(W)$ that makes the synthetic control's characteristics match the treated unit's characteristics as closely as possible, using the non-negative weights $v_m$ to prioritize certain covariates $(X_m)$.

$v_m$ is a weight that reflects the relative importance assigned to each of the predictor variables when the difference between $X_1 \text{ and } X_0W$ is measured. It is crucial that the synthetic control closely reproduces the values that variables with large predictive power on the outcome of interest take for the unit affected by the intervention. Therefore, variables with high predictive power logically receive high $v$-weights.

The $v$-weights therefore scale the influence of a potential mismatch between the pre-treatment case of interest and the synthetic control. The unit weights $w_j$, however, determine the contribution of each single unit $(j=2 \text{ to } j=J+1)$ in our data (or donor pool) to the overall value in the synthetic control condition. Therefore, the $v$-weights scale the relative importance of the $w_j$ weights for a variable $m$. Selecting a high value for $v$ also leads to a stronger influence of an individual unit's ($j$) difference between treatment and synthetic control. Where variables are assumed to receive a higher weight, values of $w_j$ should also be chosen more carefully. Selecting low values for $v$ leads to lower influence of differences in pre-treatment characteristics of the treated and the synthetic control.
...
Therefore, we can answer the question of what would happen if the weights "could" be negative or sum to something different than one, using just some basic statistics definitions. A mathematical average (or mean) is the sum of all numbers in a set divided by the count of how many numbers are in that set. A weighted average is the sum of a set of numbers where each number has first been multiplied by its own "importance" or "weight" (as explained in the paper). When the weights are negative or greater than one, it is no longer a weighted average but extrapolation, where a weight greater than one helps to project beyond, and a negative weight means to subtract that unit's contribution, creating a value that has a negative effect.

According to the authors, this is exactly what we see in regression-based approaches. Although they use synthetic comparison units having coefficients that sum to one as well, the regression approach **does not** restrict the coefficients of the linear combination that define the comparison unit to be between zero and one. If we now assume that our individual weights $(w_j)$ in the synthetic control term could also have values beyond $[0, 1]$, this would mean that we would just have a simple linear regression model whose weights extrapolate to produce a perfect fit in order to minimize the error. The findings of the paper show, however, that extrapolation is not necessary in the context of this study, since there exists a synthetic control that closely fits the values of the characteristics of the units and that does not extrapolate outside the support of the data.

---

## (c) Estimation, Balance, and Model Fit
...
balance_df <- data.frame(
  Predictor = c("GDP per capita", "Trade openness", "Inflation rate", 
                "Industry share", "Investment rate", "Schooling"),
...
Looking at the graphical representation of the trends in GDP per capita of West Germany and the synthetic control, we can see that the trends during the pre-treatment period are almost perfectly similar. In combination with the balance table, we can confidently state that there exists a set of industrialized countries that can perfectly reconstruct the pre-treatment path of key economic predictors during the pre-intervention period. As discussed above, the estimate is the post-intervention gap between the treated unit ($Y_{1t}$) and the synthetic control ($\sum w_j^* Y_{jt}$), and we see that this difference is extremely close to zero over the whole pre-treatment period, confirming the excellent match.

During the first couple of years of the post-treatment period (1990-1992), this difference is not that pronounced, even being slightly positive as indicated by the gap plot, although it starts getting substantially larger in the following years as the two lines diverge. These findings suggest a pronounced negative and growing impact of reunification on income in West Germany, with the synthetic control indicating that West Germany's GDP would have been much higher without the intervention.

The permutation test results from Table 7 and the corresponding figure further solidify the statistical credibility of the finding. The test confirms the causal claim that the West Germany RMSPE Ratio, where the post-reunification fit was over 20 times worse than the pre-reunification fit (20.646), is a massive outlier. When the same synthetic control method was applied to all 16 control countries as a placebo, zero of them produced a ratio greater than or equal to West Germany's, leading to an approximate p-value of 0.000.

Visually, the figure confirms that the black line (West Germany's gap) is tightly clustered around zero with the gray placebo lines before 1990. However, immediately following the intervention, the black line plunges dramatically and consistently to the bottom of the graph, falling far outside the distribution of the gray placebo lines. This means the negative economic effect on West Germany is highly unique and unlikely to be due to random chance, reinforcing the conclusion that it is a highly significant causal effect.

---

## (e) Robustness Check: Placebo Test with Earlier Treatment Year

### Tasks:

1. Conduct placebo treatment year **before** actual 1990 treatment (e.g., 1975)
2. Re-estimate synthetic control and plot the gap
3. What does pre-treatment gap behavior tell you about the parallel-trajectory assumption?
...
As the section above shows, we find a large negative effect on West Germany's GDP per capita in the years after the reunification intervention, and our robustness checks proved to strongly support this finding. Since our estimate is the post-intervention gap between the actual and the synthetic line, we should definitely not find a division in the trajectory pre-treatment.

Looking at our plotted placebo results, we still see an almost perfect fit for the "new" pre-treatment period (1960–1974). This fit also stays remarkably accurate after our false 1975 intervention, slightly diverging a bit more over time but generally holding the line. This is a crucial finding that confirms the model's predictive power.

Finally, also for this placebo graph, we see the diverging outcome paths in GDP per capita around the actual intervention in 1990, with the clear diverging trends beginning then. That's a fascinating result and probably the most important one as well, since this time we fixed the weights at a much earlier point (1975) in the OECD donor pool, using less economic data. Even though we just used a smaller time period to construct the counterfactual West Germany, we still see a large, convincing effect in both trajectories around the true intervention period.

Since in comparative political research it is always hard to construct counterfactuals and comparison units, especially when we're looking at aggregate entities like countries, we would naturally assume that when looking at history, there are just too many unobserved or confounding events that could mess up the trends of the actual and synthetic control unit. However, we can clearly see that, no matter what, the placebo condition still serves as an extremely accurate representation of the real, un-intervened trend, even for 15 years after the false 1975 intervention. This proves the synthetic control is robust enough to handle the complexity of real-world history and opens the door for much wider applications.

Another extremely interesting finding is that we could possibly also find interventions bottom-up, by just looking at the post-treatment gap between the economic variables from some synthetic control and an actual aggregate entity's trend. When we see sudden dips or spikes in the economic performance (the growing post-treatment gap), that signal could point to an unattributed or unstudied policy shock that demands investigation. It essentially flips the method on its head, using the synthetic control as an anomaly detector to guide new research in a wide set of fields.
