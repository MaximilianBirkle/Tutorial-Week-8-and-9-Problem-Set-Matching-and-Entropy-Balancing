...
# synth-estimation chunk
# Prepare data
# Identify West Germany's index
...
invisible(capture.output({
  ...
  # Optimization - moved inside same invisible block
  synth_out <- synth(data.prep.obj = dataprep_out, method = "BFGS")
}))
```

```{r synth-balance-table}
# synth-balance-table chunk
# Create balance table for pre-treatment predictors (GDP, trade opennesss, 
# inflation, schooling, investment)
...
balance_df <- data.frame(
  ...
  Synthetic = balance_table$tab.pred[, 2],  # Simplified column name
  OECD = balance_table$tab.pred[, 3]        # Simplified column name
)
...
```{r country-weights-table, results='asis'}
# Task 3: Report non-zero weights

# Exclude the treated unit (West Germany)
donor_names <- dataprep_out$names.and.numbers[-1, 1]
...
#Optional: visualize country weights
# Create a lookup table from dataprep_out
...
ggplot(weights_df, aes(x = reorder(Country, Weight), y = Weight)) +
...
# --- Task (d) 2/3/4: Calculate and Present Metrics in a Table ---

# Define time periods for calculating average effects and RMSPE
all_post_years <- 1990:2003
first_few_years <- 1990:1991
years <- as.numeric(rownames(dataprep_out$Y1plot))
pre_period <- years < 1990
post_period <- years >= 1990

# Calculate metrics
avg_gap_full_post <- mean(gaps[rownames(dataprep_out$Y1plot) %in% all_post_years])
avg_gap_first_years <- mean(gaps[rownames(dataprep_out$Y1plot) %in% first_few_years])
pre_rmspe <- sqrt(mean(gaps[pre_period]^2))
post_rmspe <- sqrt(mean(gaps[post_period]^2))
...
results_df <- data.frame(
  Metric = c(
    ...
    "Average Initial Gap (1990–1991)",
    "Pre-treatment RMSPE (1960–1989)",
    "Post-treatment RMSPE (1990–2003)",
...
results_table <- knitr::kable(
  results_df,
  digits = 2,
  col.names = c("Metric", "Value (USD / Ratio)"),
  caption = "Summary of Estimated Treatment Effects and Fit Metrics"
)
...
# Define the placebo year (e.g., 1975)
placebo_year <- 1975
west_germany_id <- unique(repgermany[repgermany$country == "West Germany", "index"]) # Re-define ID if needed

# 1. Prepare data for the placebo run
invisible(capture.output({
  dataprep_placebo <- dataprep(
    foo = as.data.frame(repgermany),
    predictors = c("gdp", "trade", "infrate", "industry"),
    predictors.op = "mean",
    # Standard predictors use the period prior to T_0=1975 (1965-1974)
    time.predictors.prior = 1965:1974, 
    special.predictors = list(
      # KEEP the original, data-available time period for static predictors
      # If data for 'invest80' and 'schooling' starts around 1980, use that range:
      list("invest80", 1980:1985, "mean"), 
      list("schooling", 1980:1985, "mean")
    ),
    dependent = "gdp",
    ...
    treatment.identifier = west_germany_id,
    controls.identifier = setdiff(unique(repgermany$index), west_germany_id),
    time.optimize.ssr = 1960:(placebo_year - 1), # Optimization up to 1974
    time.plot = 1960:2003
  )

  # 2. Re-estimate Synthetic Control
  synth_placebo <- synth(data.prep.obj = dataprep_placebo, method = "BFGS")
}))

# 3. Plot the gap for the placebo run (main requirement)
gaps.plot(
